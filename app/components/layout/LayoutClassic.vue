    ...
    <div class="flex">
      ...
      <aside
        ...
        :style="{
          ...
          boxShadow:
            '4px 0 24px -2px rgba(0, 0, 0, 0.03), 1px 0 0 rgba(0, 0, 0, 0.02)'
        }"
      >
        <!-- Collapse/Expand Toggle -->
        <button
          class="absolute -right-3 top-1/2 -translate-y-1/2 z-40 flex items-center justify-center w-6 h-6 rounded-full bg-(--ui-bg-elevated) border border-(--ui-border) text-(--ui-text-dimmed) hover:text-primary-500 hover:border-primary-300 transition-all duration-200 cursor-pointer"
          @click="toggleSidebar"
        >
          <Icon
            :icon="
          ...
          <div class="px-2 pb-1">
            ...
            >
              <template #trigger>
                <NuxtLink
                  :to="settingsItem.to"
                  class="mb-1 flex h-8 w-full items-center justify-center rounded-[10px] text-(--ui-text-dimmed) transition-colors hover:bg-(--ui-bg-muted)/70 hover:text-(--ui-text)"
                  :class="
