...
const items = computed(() => {
  const base = [
    ...
    { label: 'AI 配置', icon: 'lucide:brain', to: '/models' },
    { label: 'AI 用量', icon: 'lucide:activity', to: '/settings/usage' },
...
<template>
  <nav class="fixed inset-x-0 bottom-0 z-40 lg:hidden animate-slide-up">
    <div
      class="card-glass mx-3 mb-3 flex items-center justify-around rounded-full px-2 py-1.5"
    >
      <NuxtLink
        v-for="item in items"
        :key="item.to"
      ...
      >
        <Icon
          ...
          :class="isActive(item.to) ? 'animate-bounce-in' : ''"
        />
