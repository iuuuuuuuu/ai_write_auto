...
<template>
  <div class="min-h-screen bg-(--ui-bg)">
    <div class="flex">
      <!-- Sidebar (borderless - no shadow, no border) -->
      <aside
      ...
      >
        ...
        <button
          class="absolute -right-3 top-1/2 -translate-y-1/2 z-40 flex items-center justify-center w-6 h-6 rounded-full bg-(--ui-bg) border border-(--ui-border)/30 text-(--ui-text-dimmed) hover:text-primary-500 hover:border-primary-300 transition-all duration-200 cursor-pointer opacity-0 hover:opacity-100 focus:opacity-100"
          @click="toggleSidebar"
        >
          <Icon
            :icon="
        ...
        <div class="flex flex-col flex-1 min-h-0 overflow-hidden">
          ...
          <div
            ...
            :class="
              settings.sidebarExpanded ? 'justify-start' : 'justify-center'
            "
          >
          ...
          <nav class="flex-1 px-2 mt-2 space-y-1">
              ...
              <template #trigger>
                <NuxtLink
                  ...
                  :class="[
                    ...
                    : 'text-(--ui-text-muted) hover:text-(--ui-text) hover:bg-(--ui-bg-muted)/50'
                  ]"
                >
                  <Icon
                    :icon="item.icon"
            ...
            <template v-if="isAdmin">
              <div class="my-2 mx-2.5 h-px bg-(--ui-border)/20" />
              <NTooltip
                placement="right"
                :disabled="settings.sidebarExpanded"
                :show-arrow="false"
          ...
          <div class="px-2 pb-1">
            ...
            >
              <template #trigger>
                <NuxtLink
                  :to="settingsItem.to"
                  class="mb-1 flex h-8 w-full items-center justify-center rounded-[10px] text-(--ui-text-dimmed) transition-colors hover:bg-(--ui-bg-muted)/50 hover:text-(--ui-text)"
                  :class="
            ...
            <button
              class="flex items-center justify-center w-full h-8 rounded-[10px] text-(--ui-text-dimmed) hover:text-(--ui-text) hover:bg-(--ui-bg-muted)/50 transition-colors"
          ...
          <div class="px-2 py-3">
            <div
              class="flex items-center rounded-[10px] px-2.5 py-2 transition-all duration-200 hover:bg-(--ui-bg-muted)/50 cursor-pointer"
      ...
      >
        ...
        <footer
          ...
          class="shrink-0 h-8 flex items-center justify-center text-[11px] text-(--ui-text-dimmed)"
        >
          AI Novel Writing
        </footer>
      </main>
