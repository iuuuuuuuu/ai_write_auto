...
const { data: foreshadowings, refresh } = await useFetch<any[]>(
  `/api/foreshadowing`,
  {
...
const activeCount = computed(
  () => foreshadowings.value.filter((f) => f.status === 'active').length
)
...
<template>
  <div class="space-y-2">
    ...
    >
      ...
      >
        <div class="flex items-start gap-2">
          <NTag
            :type="getStatusColor(f.status)"
            size="tiny"
            >{{ getStatusLabel(f.status) }}</NTag
          >
          <div class="flex-1 min-w-0">
            <p class="text-[11px] text-(--ui-text) leading-relaxed">
              {{ f.content }}
            </p>
            <p
              v-if="f.description"
              class="text-[10px] text-(--ui-text-dimmed) mt-0.5"
            >
              {{ f.description }}
            </p>
            <div class="flex items-center gap-2 mt-0.5">
              <span
                v-if="f.chapterNumber"
                class="text-[9px] text-(--ui-text-dimmed)"
                >第{{ f.chapterNumber }}章铺设</span
              >
              <span
                v-if="f.resolvedAtChapter"
                class="text-[9px] text-emerald-500"
                >第{{ f.resolvedAtChapter }}章回收</span
              >
            </div>
          </div>
          <div
            class="opacity-0 group-hover:opacity-100 flex items-center gap-1 shrink-0"
          >
            <button
              v-if="f.status === 'active'"
              class="text-(--ui-text-dimmed) hover:text-emerald-500"
              title="标记回收"
              @click="resolveForeshadowing(f.id)"
            >
              <Icon
            ...
            <button
              class="text-(--ui-text-dimmed) hover:text-red-500"
              title="删除"
              @click="deleteForeshadowing(f.id)"
            >
              <Icon
                icon="lucide:trash-2"
                class="w-3 h-3"
    ...
    <NModal
      ...
      preset="card"
      :title="editingId ? '编辑伏笔' : '添加伏笔'"
      style="max-width: 500px"
    >
      <div class="space-y-3">
        <div>
          <label class="text-xs font-medium text-(--ui-text-dimmed)"
            >伏笔内容 *</label
          >
          <NInput
