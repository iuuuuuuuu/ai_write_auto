...
<template>
  <div>
    ...
    <div class="mb-5">
      <h1 class="text-xl font-bold text-(--ui-text-highlighted)">{{ t('auth.registerTitle') }}</h1>
      <p class="mt-1 text-sm text-(--ui-text-muted)">创建账号开始你的创作之旅</p>
    </div>

    <Transition name="slide-fade">
      <div v-if="showError"
           class="mb-3 flex items-center gap-2 p-2.5 rounded-lg text-sm"
    ...
    <form class="space-y-3" @submit.prevent="handleRegister">
      ...
      <div class="space-y-1">
        <label class="block text-[11px] font-semibold text-(--ui-text-muted) uppercase tracking-wider">{{ t('auth.confirmPassword') }}</label>
    ...
    <p class="mt-4 text-center text-xs text-(--ui-text-dimmed)">
      {{ t('auth.hasAccount') }}
      <NuxtLink to="/login" class="text-primary-600 dark:text-primary-400 hover:underline font-medium">{{ t('auth.loginButton') }}</NuxtLink>
    </p>
  </div>
</template>