...
const { data: chapters } = await useFetch(`/api/novels/${novelId.value}/chapters`)
...
const sortedChapters = computed(() => {
  ...
  return [...(chapters.value as any[])].sort((a, b) => a.chapterNumber - b.chapterNumber)
})

const currentChapter = computed(() => sortedChapters.value[currentChapterIndex.value])
...
<template>
  <div class="min-h-screen bg-white dark:bg-gray-950">
    <!-- Header -->
    <div class="sticky top-0 z-10 bg-white/80 dark:bg-gray-950/80 backdrop-blur border-b border-gray-200 dark:border-gray-800">
      <div class="max-w-3xl mx-auto px-4 py-3 flex items-center justify-between">
        <div class="flex items-center gap-3">
          <UButton variant="ghost" icon="i-lucide-arrow-left" size="sm" :to="`/novels/${novelId}`" />
          <div class="min-w-0">
            <p class="text-sm font-medium text-gray-900 dark:text-white truncate">{{ (novel as any)?.title }}</p>
            <p class="text-xs text-gray-400">{{ currentChapter?.title }}</p>
          </div>
        </div>
        <div class="flex items-center gap-2">
          <UButton size="xs" variant="ghost" icon="i-lucide-minus" @click="fontSize = Math.max(14, fontSize - 2)" />
          <span class="text-xs text-gray-400 w-8 text-center">{{ fontSize }}</span>
          <UButton size="xs" variant="ghost" icon="i-lucide-plus" @click="fontSize = Math.min(28, fontSize + 2)" />
          <UButton
            size="xs"
    ...
    <!-- Scroll Mode -->
    <div v-if="readingMode === 'scroll'" class="max-w-3xl mx-auto px-6 py-10">
      <div v-for="(ch, idx) in sortedChapters" :key="ch.id" class="mb-16">
