...
<template>
  <div class="flex h-screen overflow-hidden bg-(--ui-bg)">
    ...
    >
      <!-- Top: Logo area -->
      <div class="flex h-12 items-center justify-center shrink-0 border-b border-(--ui-border)/50">
        <div class="flex items-center gap-2 overflow-hidden">
          <div class="flex size-8 shrink-0 items-center justify-center rounded-xl bg-primary-500/10">
            <Icon icon="lucide:shield" class="size-4 text-primary-500" />
          </div>
          <span class="admin-sidebar-text text-sm font-semibold text-(--ui-text-highlighted) whitespace-nowrap">
            管理端
          </span>
        </div>
      </div>
      ...
      <nav class="flex-1 overflow-y-auto overflow-x-hidden py-2 px-2">
        <template v-for="(group, gIdx) in adminNavGroups" :key="group.title">
          <div v-if="gIdx > 0" class="my-2 mx-2 h-px bg-(--ui-border)/60" />
          <div class="space-y-0.5">
            ...
            >
              ...
              <span class="admin-sidebar-text text-[13px] font-medium whitespace-nowrap">
                {{ item.label }}
              </span>
            </NuxtLink>
          </div>
      ...
      <div class="shrink-0 border-t border-(--ui-border)/50 p-2 space-y-0.5">
        ...
        >
          <div class="flex size-9 shrink-0 items-center justify-center">
            <Icon icon="lucide:arrow-left" class="size-[18px]" />
          </div>
          <span class="admin-sidebar-text text-[13px] font-medium whitespace-nowrap">返回前台</span>
        </NuxtLink>
        <button
        ...
        >
          <div class="flex size-9 shrink-0 items-center justify-center">
            <Icon icon="lucide:log-out" class="size-[18px]" />
          </div>
          <span class="admin-sidebar-text text-[13px] font-medium whitespace-nowrap">退出登录</span>
        </button>
      </div>
    </aside>
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
      <!-- Top bar -->
      <header class="flex h-10 shrink-0 items-center justify-between border-b border-(--ui-border) px-4">
        <div class="flex items-center gap-3">
          ...
          <span class="text-sm font-medium text-(--ui-text)">{{ currentPageTitle }}</span>
        </div>
        <div class="flex items-center gap-2">
          <span class="text-xs text-(--ui-text-dimmed)">{{ user?.username }}</span>
          <button
    ...
    <Teleport to="body">
      <Transition name="overlay">
        <div v-if="sidebarOpen" class="fixed inset-0 z-50 lg:hidden">
          ...
          <aside class="absolute inset-y-0 left-0 w-[260px] bg-(--ui-bg-elevated) border-r border-(--ui-border) flex flex-col">
            <div class="flex h-12 items-center justify-between px-4 border-b border-(--ui-border)/50">
              <div class="flex items-center gap-2">
                <div class="flex size-7 items-center justify-center rounded-lg bg-primary-500/10">
                  <Icon icon="lucide:shield" class="size-3.5 text-primary-500" />
                </div>
                <span class="text-sm font-semibold text-(--ui-text-highlighted)">管理端</span>
              </div>
              <button
            ...
            <nav class="flex-1 overflow-y-auto py-3 px-3">
              <template v-for="(group, gIdx) in adminNavGroups" :key="group.title">
                <div v-if="gIdx > 0" class="my-2.5 mx-1 h-px bg-(--ui-border)/60" />
            ...
            <div class="shrink-0 border-t border-(--ui-border)/50 p-3 space-y-0.5">
              <NuxtLink
                to="/dashboard"
