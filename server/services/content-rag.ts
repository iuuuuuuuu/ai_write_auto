...
import { isActiveChapterRef, isPopulatedChapter } from '../utils/chapter-refs'
...
export async function indexWorldDetail(
  novelId: number,
  content: string
): Promise<void> {
  if (!isEmbeddingReady() || !content.trim()) return
...
export async function indexForeshadowing(
  ...
  await upsertVector({
    ...
    chapterId,
    novelId,
    contentType: 'foreshadowing',
    content,
    embedding
  })
}

export async function indexChapterSummary(
  novelId: number,
  chapterId: number,
  summary: string
): Promise<void> {
...
export async function retrieveRelevant(
  ...
  const characters =
    characterIds.length > 0 ?
      await em.find(CharacterSchema, { id: { $in: characterIds } })
    : []
...
export async function retrieveCharacterContext(
  novelId: number,
  query: string,
  topK: number = 10
) {
...
> {
  ...
  return active
    .filter((f) => isActiveChapterRef(f.chapter))
    .map((f) => ({
      content: f.content,
      description: f.description,
      chapterNumber:
        isPopulatedChapter(f.chapter) ? (f.chapter.chapterNumber ?? null) : null
    }))
}
...
export async function reindexNovel(novelId: number): Promise<number> {
  ...
  for (const char of characters) {
    ...
    const profile = [char.description, char.traits, char.relationships]
      .filter(Boolean)
      .join(' ')
    if (profile) {
      await indexCharacterEvent(
  ...
  const assignments = await em.find(
    ChapterCharacterSchema,
    { chapter: { novel: novelId, deletedAt: null } },
    { populate: ['chapter', 'character'] }
  )
  for (const cc of assignments as any[]) {
    if (cc.chapterStory) {
      const charId =
        typeof cc.character === 'object' ? cc.character.id : cc.character
      const chapterId =
        typeof cc.chapter === 'object' ? cc.chapter.id : cc.chapter
      await indexCharacterEvent(
        charId,
        chapterId,
        novelId,
  ...
  for (const pp of filterUsablePlotPoints(plotPoints)) {
    if (pp.description) {
      const chapterId =
        pp.chapter ?
          typeof pp.chapter === 'object' ?
            pp.chapter.id
          : pp.chapter
        : null
      await indexPlotEvent(pp.id, novelId, chapterId, pp.description)
      indexed++
    }
  }
  const novel = await em.findOne(NovelSchema, { id: novelId })
  if (novel?.worldSetting) {
    await indexWorldDetail(novelId, novel.worldSetting)
    indexed++
  }
  const foreshadowings = await em.find(
    ForeshadowingSchema,
    { novel: novelId },
    { populate: ['chapter'] }
  )
  for (const f of foreshadowings.filter((f) => isActiveChapterRef(f.chapter))) {
    const chapterId =
      f.chapter ?
        typeof f.chapter === 'object' ?
          f.chapter.id
        : f.chapter
      : null
    await indexForeshadowing(f.id, novelId, chapterId, f.content)
    indexed++
  }
  ...
  for (const ch of chapters) {
    if (ch.summary) {
      await indexChapterSummary(novelId, ch.id, ch.summary)
      indexed++
    }
  }
  return indexed
