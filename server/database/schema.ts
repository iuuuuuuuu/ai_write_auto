...
export const aiConfigs = sqliteTable('ai_configs', {
  ...
  userId: integer('user_id')
    .notNull()
    .references(() => users.id),
  name: text('name').notNull().default('默认模型'),
  purpose: text('purpose', {
    enum: ['generation', 'extraction', 'consistency_check', 'style_analysis']
  }).notNull(),
...
export const chapters = sqliteTable('chapters', {
  ...
  status: text('status', { enum: ['draft', 'generated', 'edited', 'final'] })
    .notNull()
    .default('draft'),
...
export const plotPoints = sqliteTable('plot_points', {
  ...
  description: text('description').notNull(),
  type: text('type', {
    enum: ['setup', 'conflict', 'resolution', 'twist']
  }).notNull(),
  status: text('status', { enum: ['introduced', 'developing', 'resolved'] })
    .notNull()
    .default('introduced'),
  createdAt: integer('created_at', { mode: 'timestamp' })
    .notNull()
...
export const promptTemplates = sqliteTable('prompt_templates', {
  ...
  category: text('category', {
    enum: ['generation', 'rewrite', 'expand', 'custom']
  }).notNull(),
