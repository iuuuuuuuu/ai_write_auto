...
const TYPE_PROMPTS: Record<string, string> = {
  dialogue:
    '你是一位专业的小说作家。请根据上下文，生成一段精彩的人物对话。对话要自然、有个性、能推动情节。必须使用自然的段落换行（用空行分段），不要生成一整段没有换行的文字。只返回对话内容，不要其他说明。',
  description:
    '你是一位专业的小说作家。请根据上下文，生成一段环境或场景描写。注重感官细节、氛围营造和空间感。必须使用自然的段落换行（用空行分段），不要生成一整段没有换行的文字。只返回描写内容，不要其他说明。',
  action:
    '你是一位专业的小说作家。请根据上下文，生成一段动作场景。节奏紧凑、画面感强、动词精准。必须使用自然的段落换行（用空行分段），不要生成一整段没有换行的文字。只返回动作内容，不要其他说明。',
  monologue:
    '你是一位专业的小说作家。请根据上下文，生成一段角色内心独白。深入角色心理、情感真挚、推动人物塑造。必须使用自然的段落换行（用空行分段），不要生成一整段没有换行的文字。只返回独白内容，不要其他说明。'
}
...
export default defineEventHandler(async (event) => {
  ...
  const context =
    data.contextBefore || chapterContent.slice(-CONTEXT_TRUNCATE_FRAGMENT)
