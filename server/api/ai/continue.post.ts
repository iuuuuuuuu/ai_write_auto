...
export default defineEventHandler(async (event) => {
  ...
  if (!isEnding) {
    const { retrievedNotes } = await gatherRelevantContext(em, {
      ...
      userId: auth.userId,
      intent: '续写衔接',
      seed: [chapter.title, data.contextBefore.slice(-200), data.direction]
        .filter(Boolean)
        .join(' '),
      depth: 'query-only',
      topK: 8
  ...
  try {
    ...
    if (foreshadowing.length) {
      foreshadowSection = '\n## 当前活跃伏笔\n'
      for (const f of foreshadowing.slice(0, 5))
        foreshadowSection += `- ${f.content}\n`
    }
  } catch {}
...
${buildProseProtocolRules(novel)}${novel.styleGuide ? `\n\n## 风格指南\n${novel.styleGuide}` : ''}`
        : `你是一位专业的小说作家。请根据上下文，在光标位置之后继续创作小说内容。保持原有的风格、语气和叙事节奏，确保情节连贯自然。必须使用自然的段落换行（用空行分段），不要生成一整段没有换行的文字。只返回续写的内容，不要其他说明。
