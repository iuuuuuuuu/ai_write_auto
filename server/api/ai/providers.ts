...
function serializeProvider(provider: any) {
  return {
    ...
    lastCheckAt: provider.lastCheckAt?.toISOString?.() || provider.lastCheckAt || null,
...
export default defineEventHandler(async (event) => {
  ...
  if (method === 'GET') {
    const providers = await em.find(AiProviderSchema, { user: auth.userId }, {
      orderBy: { enabled: 'DESC', name: 'ASC' }
    })

    const models = await em.find(AiModelSchema, { user: auth.userId }, {
      orderBy: { enabled: 'DESC', name: 'ASC' }
    })
  ...
  if (method === 'POST') {
    ...
    if (data.id) {
      const existing = await em.findOne(AiProviderSchema, { id: data.id, user: auth.userId })
      if (!existing) {
  ...
  if (method === 'DELETE') {
    ...
    const existing = await em.findOne(AiProviderSchema, { id, user: auth.userId })
    if (!existing) {
      throw createError({ statusCode: 404, message: '供应商不存在' })
    }

    const modelCount = await em.count(AiModelSchema, { provider: id, user: auth.userId })
    if (modelCount > 0) {
      throw createError({ statusCode: 409, message: `该供应商下有 ${modelCount} 个模型，请先删除模型` })
    }
