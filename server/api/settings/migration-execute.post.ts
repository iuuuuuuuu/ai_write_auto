...
} from '../../database/migration-executor'
...
const migrateSchema = z.object({
  ...
  confirmEmptyTarget: z.literal(true)
})

interface MigrationExecuteResponse extends DatabaseMigrationResult {
...
export default defineEventHandler(
  async (event): Promise<MigrationExecuteResponse> => {
    requireAdmin(event)

    const body = await readBody(event)
    const data = migrateSchema.parse(body)
    const target = normalizeTargetConfig(data.target as DbConfig)
    const source = readDbConfig() || getEffectiveDbConfig()

    if (isSameDatabaseConfig(source, target)) {
      throw createError({
        statusCode: 400,
        message: 'Target database is the same as the current database'
      })
    }
    ...
    try {
      ...
      const result = await migrateDatabaseRows(
        sourceOrm,
        source,
        targetOrm,
        target
