...
const { mockCallAi, mockRetrieve, mockResolvePlanning, mockEmbeddingReady, mockFlagEnabled } =
  vi.hoisted(() => ({
    mockCallAi: vi.fn(),
    mockRetrieve: vi.fn(),
...
const em = {} as any
const fakeCfg = {
  id: 1, configId: 1, modelId: 1,
  apiUrl: 'http://x', apiKey: 'k', model: 'm', temperature: '0.3', maxTokens: 256
}
const sampleNote = {
  content: '主角的近况', contentType: 'profile', chapterId: null, characterName: '主角', score: 0.9
}
...
describe('gatherRelevantContext', () => {
  beforeEach(() => {
    ...
    mockCallAi.mockResolvedValue({ content: '["查询A","查询B"]', inputTokens: 10, outputTokens: 5 })
  })

  it('seed-only: 不调规划模型，用 seed 直接检索', async () => {
    const r = await gatherRelevantContext(em, {
      novelId: 1, userId: 1, intent: '测试', seed: '种子查询', depth: 'seed-only'
    })
    expect(mockCallAi).not.toHaveBeenCalled()
    expect(mockRetrieve).toHaveBeenCalledWith(1, '种子查询', 10, undefined)
    expect(r.retrievedNotes).toHaveLength(1)
    expect(r.retrievedNotes[0]).toMatchObject({ content: '主角的近况', characterName: '主角' })
  })

  it('query-only: 调一次规划模型，按产出的每条 query 检索', async () => {
    const r = await gatherRelevantContext(em, {
      novelId: 1, userId: 1, intent: '测试', seed: 's', depth: 'query-only'
    })
  ...
  it('query-only: 模型返回非法 JSON 时回落到 seed 检索', async () => {
    mockCallAi.mockResolvedValue({ content: '对不起我无法规划', inputTokens: 3, outputTokens: 2 })
    const r = await gatherRelevantContext(em, {
      novelId: 1, userId: 1, intent: '测试', seed: '兜底种子', depth: 'query-only'
    })
    expect(mockCallAi).toHaveBeenCalledTimes(1)
  ...
  it('full 在 v1 等价 query-only（仍走规划模型）', async () => {
    await gatherRelevantContext(em, {
      novelId: 1, userId: 1, intent: '测试', seed: 's', depth: 'full'
    })
    expect(mockCallAi).toHaveBeenCalledTimes(1)
  })
  ...
  it('无 planning 配置时回落到 seed 检索（不抛错）', async () => {
    ...
    const r = await gatherRelevantContext(em, {
      novelId: 1, userId: 1, intent: '测试', seed: '兜底', depth: 'query-only'
    })
    expect(mockCallAi).not.toHaveBeenCalled()
  ...
  it('embedding 未就绪时只返回 extraNotes，不检索不调模型', async () => {
    ...
    const extra = [{ content: 'shared', contentType: 'chapter_summary', chapterId: 2 }]
    const r = await gatherRelevantContext(em, {
      novelId: 1, userId: 1, intent: '测试', seed: 's', depth: 'query-only', extraNotes: extra
    })
  ...
  it('retrievedNotes 形状为 RagContextItem 且与 extraNotes 合并去重', async () => {
    const dup = { content: 'dup', contentType: 'profile', chapterId: null, characterName: 'A', score: 0.5 }
    mockRetrieve.mockResolvedValue([dup]) // 两条 query 各返回同一条
    const extra = [{ content: 'dup', contentType: 'profile', chapterId: null, characterName: 'A' }] // 又是同一条
    const r = await gatherRelevantContext(em, {
      novelId: 1, userId: 1, intent: '测试', seed: 's', depth: 'query-only', extraNotes: extra
    })
    expect(r.retrievedNotes).toHaveLength(1)
    expect(r.retrievedNotes[0]).toMatchObject({ content: 'dup', contentType: 'profile' })
  })

  it('contentType 过滤会透传给 retrieveRelevant', async () => {
    await gatherRelevantContext(em, {
      novelId: 1, userId: 1, intent: '测试', seed: 's', depth: 'seed-only', contentType: 'profile'
    })
